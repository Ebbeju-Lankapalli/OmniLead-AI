...
class CallRecordingBase(ORMModel):
    ...
    def validate_filename(
        ...
        if "." not in cleaned:
            raise ValueError(
                "Audio filename must include an extension."
            )

        extension = cleaned.rsplit(
            ".",
            1,
        )[-1].lower()
    ...
    def validate_file_size(
        ...
        if value > max_bytes:
            raise ValueError(
                ...
                f"{DEFAULT_MAX_UPLOAD_SIZE_MB} MB "
                f"upload limit."
            )
...
class CallRecordingSummary(ORMModel):
    """Call recording representation for call history."""
    ...
    # Persisted AI intelligence associated with
    # this specific call recording.
    intelligence: (
        "CallIntelligenceResponse | None"
    ) = None
...
class CallTranscriptionFailure(ORMModel):
    ...
    def normalize_error(
        ...
        if not cleaned:
            raise ValueError(
                "Transcription error cannot be empty."
            )
